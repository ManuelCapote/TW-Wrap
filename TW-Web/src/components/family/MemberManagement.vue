...
<template>
  <div data-role="section" data-section-type="members-list" class="space-y-6">
    <div v-if="familyStore.isLoading" data-loading="true" data-role="grid" data-grid-type="members" class="grid gap-4 md:grid-cols-2">
      <div
        v-for="skeleton in 2"
        :key="skeleton"
        data-role="skeleton"
        data-skeleton-type="member-card"
    ...
    <div v-else data-role="grid" data-grid-type="members" class="grid gap-4 md:grid-cols-2">
      <div
        v-for="member in members"
        :key="member.id"
        data-role="member-card"
        :data-member-id="member.id"
        :data-is-current-user="member.id === currentUserId"
      ...
      >
        <header data-role="card-header" class="flex items-start gap-4">
          <div data-role="avatar" class="flex h-12 w-12 items-center justify-center overflow-hidden rounded-full bg-primary-soft text-lg text-primary">
            <img
          ...
          <div data-role="member-info" class="flex-1 space-y-2">
            <div class="flex flex-wrap items-center gap-2">
              <h3 data-role="member-name" class="text-base font-semibold text-text">{{ member.name }}</h3>
              <span
                v-if="member.id === currentUserId"
                data-role="badge"
                data-badge-type="status"
                data-status-type="current-user"
            ...
            <div class="flex flex-wrap items-center gap-2 text-xs text-text-secondary">
              <span data-role="member-email" class="inline-flex items-center gap-1">
        ...
        <div data-role="member-metadata" class="mt-5 space-y-3 text-sm text-text-secondary">
          <div class="flex flex-wrap items-center gap-2">
            <span class="text-xs font-semibold text-text-tertiary">Role</span>
            <RoleBadge :role="member.role" size="small" />
          </div>
          <div data-role="badge" data-badge-type="date" class="flex items-center gap-2 text-xs text-text-tertiary">
    ...
    <div
      ...
      data-role="grid"
      data-grid-type="stats"
      class="grid gap-4 rounded-2xl border border-border bg-surface px-5 py-6 shadow-md shadow-black/20 md:grid-cols-3"
    >
      <div data-role="stat-card" data-stat-type="members" class="rounded-xl border border-border bg-background px-4 py-4 text-center">
        <p data-role="stat-value" class="text-3xl font-semibold text-primary">{{ members.length }}</p>
        <p data-role="stat-label" class="mt-1 text-xs font-semibold uppercase tracking-[0.3em] text-text-tertiary">Total</p>
        <p data-role="stat-description" class="mt-1 text-xs text-text-secondary">Members in this family</p>
      </div>
      <div data-role="stat-card" data-stat-type="admins" class="rounded-xl border border-border bg-background px-4 py-4 text-center">
        <p data-role="stat-value" class="text-3xl font-semibold text-primary">{{ familyStore.adminMembers.length }}</p>
        <p data-role="stat-label" class="mt-1 text-xs font-semibold uppercase tracking-[0.3em] text-text-tertiary">Admins</p>
        <p data-role="stat-description" class="mt-1 text-xs text-text-secondary">Manage invites and settings</p>
      </div>
      <div data-role="stat-card" data-stat-type="regular-members" class="rounded-xl border border-border bg-background px-4 py-4 text-center">
        <p data-role="stat-value" class="text-3xl font-semibold text-primary">{{ familyStore.regularMembers.length }}</p>
        <p data-role="stat-label" class="mt-1 text-xs font-semibold uppercase tracking-[0.3em] text-text-tertiary">Members</p>
        <p data-role="stat-description" class="mt-1 text-xs text-text-secondary">Can browse and contribute</p>
      </div>
    </div>
  </div>
</template>
